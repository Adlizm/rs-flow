...
impl Package {
    ...
    /// ```
    ///
    pub fn try_into<T: for<'a> Deserialize<'a>>(self) -> Result<T, PackageDeserializerError> {
        deserialize(self)
    }
    ...
    pub fn object<T: Into<Package>, K: Into<String>>(
        value: impl IntoIterator<Item = (K, T)>,
    ) -> Self {
        Self::Object(
            value
                .into_iter()
